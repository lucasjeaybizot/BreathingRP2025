...
%  - Extracts the respiration phase at button press only for first 75 trials,
%  - Stores results in a struct and saves them to results\figure_S3 folder.
    ...
    for participant = 1:17
        ...
        % Select first 75
        trial_idx       = trial_idx(1:75);
    
        press_samples   = round([ev(trial_idx).sample]/(2500/512));
    
        resp_75(participant).resp_phases = data_resp_bp.trial{1,1}(press_samples);
    
    end

    save('results\figure_S3\resp_75.mat', 'resp_75')
