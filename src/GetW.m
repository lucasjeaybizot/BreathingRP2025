...
%  - Extracts the respiration phase at button press only for W trials,
%  - Stores results in a struct and saves them to results\figure_S3 folder.
    ...
    for participant = 1:17
        ...
        trial_idx       = [find(strcmp({ev.value}, 'S 30'),50,'first')];
    
        press_samples   = round([ev(trial_idx).sample]/(2500/512));
    
        resp_W(participant).resp_phases = data_resp_bp.trial{1,1}(press_samples);
    
    end

    save('results\figure_S3\resp_W.mat', 'resp_W')
